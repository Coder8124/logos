...
> **This is a design document, not a description of what ships.** It records the
> shape the system is being built toward. Sections marked *(not built)* describe
> intent only — nothing in the repository implements them today. Everything else
> is either built or partly built; `brain doctor` is the honest account of what
...
of a conversion at every call. `enginetest/` is an external test package, so it
