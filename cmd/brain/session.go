...
func runNote(args []string) error {
	...
	// standing in — `brain resume`'s own empty-state message tells the user to
	// run `brain note <project> "..."` having worked the project out from the
	// cwd to print that line; the command itself refusing to do the same
	// lookup was a usage error where the obvious thing was available. Two or
	// more arguments keeps the older reading, where the first names the
	// project explicitly.
