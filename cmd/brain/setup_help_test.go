...
// `brain setup --help` used to run setup: it created ~/brain, recorded it as
...
// With no vault yet, `brain mcp install --help` answered "vault not found …
// run `brain setup` first" instead of the flags it takes.
func TestMcpInstallHelpPrintsTheFlagsInsteadOfAVaultError(t *testing.T) {
	...
	if !strings.Contains(out, "brain mcp install [--vault DIR]") {
