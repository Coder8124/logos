...
// updateCmd is `logos update` / `logos update --check`. It is the one place
...
func updateCmd(args []string) error {
	...
	if hasFlag(args, "--check") {
		...
		if rel.Version == version {
			fmt.Printf("logos %s is current\n", version)
			return nil
		}
		if !selfupdate.Newer(rel.Version, version) {
			fmt.Printf("logos %s is newer than the latest release, %s; nothing to update\n", version, rel.Version)
			return nil
		}
		fmt.Printf("logos %s → %s available\n", version, rel.Version)
		return nil
	}
