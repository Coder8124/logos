...
// vault is truth, .logos/index.db is a disposable cache, and every writer's
// promise is "delete the index, run logos index, lose nothing". This test
...
func TestVaultIsTruthAcrossEveryWriter(t *testing.T) {
	...
	t.Setenv("LOGOS_PROJECT", "truthproj")
	...
	if !strings.Contains(memOut, fact) {
		t.Errorf("a memory did not survive deleting .logos and reindexing:\nwant substring %q\ngot:\n%s", fact, memOut)
	}
	...
	for _, want := range []string{nextStep, failedTry} {
		if !strings.Contains(resumeOut, want) {
			t.Errorf("a checkpoint field did not survive deleting .logos and reindexing:\nwant substring %q\ngot:\n%s", want, resumeOut)
		}
	}
}
