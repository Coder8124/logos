...
// <vault>/.logos — which brings the vault itself into being. So doctor answered
...
// the CLI. The cost is not cosmetic: a typo in LOGOS_VAULT, or running doctor
...
func TestDoctorDoesNotCreateTheVaultItIsChecking(t *testing.T) {
	...
	if !strings.Contains(c.Fix, "logos setup") {
		t.Errorf("vault fix is %q, want it to name `logos setup`", c.Fix)
	}
}
...
func TestDoctorStillReportsWithNoVault(t *testing.T) {
	t.Setenv("LOGOS_VAULT", filepath.Join(t.TempDir(), "no-vault-here"))
...
// Telling someone to set LOGOS_VAULT is the wrong instruction in both cases
// that reach here: they either never ran setup, or they set LOGOS_VAULT already
...
func TestMissingVaultErrorDoesNotAdviseSettingAVariableThatIsAlreadySet(t *testing.T) {
	err := missingVaultError("/nowhere/logos")
	msg := err.Error()

	if !strings.Contains(msg, "/nowhere/logos") {
		t.Errorf("message %q does not name the path it looked at", msg)
	}
	if !strings.Contains(msg, "logos setup") {
