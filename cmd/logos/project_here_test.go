...
// `logos checkpoint` ends by telling the reader "`logos resume` picks it up
...
// LOGOS_PROJECT outranks the directory for the CLI exactly as it does for the
...
func TestLogosProjectOutranksTheDirectory(t *testing.T) {
	vaultDir := standIn(t, "widgets")
	t.Setenv("LOGOS_PROJECT", "gadgets")

	if err := runCheckpoint([]string{"--task", "cache layer"}); err != nil {
		t.Fatalf("checkpoint: %v", err)
	}
	if _, err := os.Stat(filepath.Join(vaultDir, "sessions", "gadgets")); err != nil {
		t.Fatalf("LOGOS_PROJECT did not win over the directory: %v", err)
	}
}
...
// `logos resume`'s own empty-state message tells a new user to run
// `logos note myapp "what you just did"` — it worked out the project from the
...
// `logos context "<task>"` standing inside a project answered "no project
// matched" and served standing context only, while `logos resume` one line
