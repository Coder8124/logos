...
// logos context --pin is the CLI's half of the tree view's control surface —
...
func TestLogosContextPinWritesADurablePathRule(t *testing.T) {
	vault := t.TempDir()
...
func TestLogosContextExcludeThenUnpinClearsTheRule(t *testing.T) {
	vault := t.TempDir()
	t.Setenv("LOGOS_VAULT", vault)
	if err := os.MkdirAll(vault, 0o755); err != nil {
		t.Fatal(err)
...
func TestLogosContextRulesWithNoVaultReportsTheStandardMissingVaultError(t *testing.T) {
