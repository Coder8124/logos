...
func runWhy(args []string) error {
	...
	if strings.TrimSpace(path) == "" {
		return fmt.Errorf("usage: logos why <file> [--limit N]")
	}
	...
	if len(mentions) == 0 {
		...
		// mentions it" is a fact about the record; "logos does not know" would
		...
		fmt.Println("recorded under a different path. `logos sessions <project>` lists what")
