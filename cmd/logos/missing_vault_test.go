...
// Pointing LOGOS_VAULT at a path that does not exist is either a typo or a
// machine where `logos setup` has not run yet. Every command has to say so.
// `logos announce quiet` and `logos think medium` did not: they created the
...
// `logos doctor` is the command a script or a pre-flight check runs to decide
...
// command that crashed before it got started. `logos ask` says so in words and
// search now does too.
func TestASearchWithNoHitsSaysSoRatherThanPrintingNothing(t *testing.T) {
	...
	t.Setenv("LOGOS_EMBED", "off") // lexical only: no runtime in a test
