...
func memoryCmd(args []string) error {
	...
	case "history":
		id := parseID(args)
		if id == 0 {
			return fmt.Errorf("usage: logos memory history <id>")
		}
	...
	case "forget":
		...
		// like `logos bootstrap` needs: the alternative is asking the user to
		...
		id := parseID(args)
		if id == 0 {
			return fmt.Errorf("usage: logos memory forget <id> | --source <name>")
		}
	...
	case "pin":
		id := parseID(args)
		if id == 0 {
			return fmt.Errorf("usage: logos memory pin <id>")
		}
	...
	case "unpin":
		id := parseID(args)
		if id == 0 {
			return fmt.Errorf("usage: logos memory unpin <id>")
		}
	...
	case "exclude":
		id := parseID(args)
		if id == 0 {
			return fmt.Errorf("usage: logos memory exclude <id>")
		}
		if err := memory.Exclude(ix.DB, id); err != nil {
			return err
		}
		fmt.Println("excluded — kept on record, never surfaced. `logos memory unpin` to reverse.")
	default:
		return fmt.Errorf("usage: logos memory [add <fact> [--project P] | forget <id> | pin <id> | unpin <id> | exclude <id> | log [--project P] | history <id> | diff | health | consolidate]")
	}
	return nil
}
...
//
//	logos memory diff [subject…] [--since YYYY-MM-DD] [--until YYYY-MM-DD] [--days N]
//
...
func memoryHealth(db *sql.DB) error {
	...
	if rep.Duplicates > 0 {
		fmt.Println("\ntip: `logos memory consolidate` folds duplicates and retires stale facts")
	}
	return nil
}
...
func memoryLog(db *sql.DB, project string, n int) error {
	...
	if len(entries) == 0 {
		if project != "" {
			...
			fmt.Println("  `logos memory log` with no --project shows every project's events together.")
			return nil
		}
	...
	if project != "" {
		if n, err := memory.UnattributedCount(db); err == nil && n > 0 {
			fmt.Printf("\n%d older events carry no project and are not shown here; `logos memory log` includes them.\n", n)
		}
	}
	return nil
}
