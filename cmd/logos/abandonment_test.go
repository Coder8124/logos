...
// thing reads as a broken tool to a first-time user. `logos note`, `logos
// resume`, `logos checkpoint` and `logos sessions` all required an explicit
// project name, even though every one of them can work it out from the
// directory the user is already standing in — exactly as `logos project-name`
// does.
func TestProjectArgFallsBackToTheDirectoryYouAreStandingIn(t *testing.T) {
	...
	defer os.Chdir(old)
	t.Setenv("LOGOS_PROJECT", "standing-here")
...
// `logos doctor` prints a FAILED row and still exits 0, which is invisible to
// anything that reads the exit code rather than the terminal — a pre-flight
// `logos doctor && logos resume proj` in a script proceeds past a vault the
...
// `logos doctor --probe && ...` walked straight past a runtime it had just been
// told was broken.
...
func TestSearchOnAnEmptyIndexSaysNothingMatched(t *testing.T) {
	...
	// openIndex creates .logos/index.db on first open; no notes are indexed.
