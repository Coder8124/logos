...
import (
	...
	"github.com/Coder8124/logos/internal/rename"
...
// to. LOGOS_PROJECT outranks the directory exactly as it does in the MCP
...
func projectFor(dir string) string {
	if name := strings.TrimSpace(os.Getenv("LOGOS_PROJECT")); name != "" {
		return name
	}
...
// `logos checkpoint` says "`logos resume` picks it up now", and a bare
// `logos resume` that answered with a usage error made the product's own
// instruction wrong.
...
// leading flag is never the project: `logos checkpoint --task ...` used to file
...
func runProjectRename(args []string) error {
	...
	if len(names) != 2 {
		return fmt.Errorf("usage: logos project rename <old> <new> [--dry-run] [--merge]")
	}
	...
	if res.Empty() && res.Merged == 0 {
		fmt.Printf("nothing filed under %q — check the name with `logos projects`\n", from)
		return nil
	}
	...
	fmt.Println("run `logos index` to refresh search over the moved notes.")
	return nil
}
