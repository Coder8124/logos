...
// `logos activity` — what the agents actually did, as opposed to what they
...
func recordActivity(args []string) error {
	...
	if event == "" {
		return fmt.Errorf("usage: logos activity record --event <HookEventName> < payload.json")
	}
	...
	if event == "PostToolUse" && e.Tool == "ExitPlanMode" {
		if plan := planText(raw); plan != "" && e.Project != "" {
			if _, err := session.SavePlan(vault, session.Plan{
				...
				// visibility surface `logos activity` reads; a warning row
				// there survives where a discarded stderr line does not.
				fmt.Fprintf(os.Stderr, "logos: could not save plan: %v\n", err)
