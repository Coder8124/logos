...
func TestHelpShortFitsAScreen(t *testing.T) {
	...
	if !strings.Contains(buf.String(), "logos help all") {
		t.Error("short help does not mention `logos help all`, so the other verbs are unreachable")
	}
}

// Demoting the general surface must not amount to losing it. Every verb main
// dispatches on has to appear in `logos help all`, so a command added later
...
func TestHelpAllListsEveryVerb(t *testing.T) {
	...
	for _, m := range verbs {
		verb := m[1]
		// Word-boundary rather than "logos <verb>": several verbs are documented
		// on a shared line, as `logos voice | listen | say <text>`.
		if !regexp.MustCompile(`\b` + verb + `\b`).MatchString(all) {
			t.Errorf("verb %q is dispatched by main but absent from `logos help all`", verb)
		}
	}
}
