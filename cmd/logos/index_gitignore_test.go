...
// `logos index` is the command every checkpoint tells the user to run, and
// the plan for a vault two people share over git depends on .logos/ never
...
func TestLogosIndexAddsAndAnnouncesTheGitignoreRuleOnAFreshVault(t *testing.T) {
	vaultDir := t.TempDir()
	...
	out := captureStdout(t, func() {
		if err := runIndex(false); err != nil {
			t.Fatalf("logos index: %v", err)
		}
	})
	if !strings.Contains(out, ".logos/") || !strings.Contains(out, ".gitignore") {
		t.Errorf("logos index did not announce writing the .gitignore rule:\n%s", out)
	}
...
// already there, so announcing it again on every single `logos index` would
// make the one useful signal (this run just changed your repo) indistinguishable
// from routine noise.
func TestLogosIndexStaysQuietAboutGitignoreOnceItIsAlreadyThere(t *testing.T) {
	vaultDir := t.TempDir()
	t.Setenv("LOGOS_VAULT", vaultDir)
	t.Setenv("LOGOS_EMBED", "off")
