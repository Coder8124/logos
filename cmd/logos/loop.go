...
func commitmentCmd(args []string) error {
	...
	// that change something, and `logos loop list` answered with a usage error —
	...
	case "add":
		...
		// user's second `logos loop` would show a list their new loop was not in
	...
	default:
		return fmt.Errorf("usage: logos loop [list | add <text> | done <id> | drop <id>]")
	}
	return nil
}

// closeLoop resolves one loop and says which. The UPDATE behind SetStatus
// matches on id and is content to match nothing, so `logos loop done 9` on a
...
func closeLoop(db *sql.DB, args []string, status secretary.Status, verb string) error {
	id := parseID(args)
	if id == 0 {
		return fmt.Errorf("usage: logos loop %s <id> — the id is the number in brackets in `logos loop`", args[0])
	}
	...
	return fmt.Errorf("no open loop [%d] — run `logos loop` to see the ids", id)
}
