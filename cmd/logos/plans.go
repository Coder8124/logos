...
// `logos plans <project>` — where an approved plan-mode plan ends up, since
// the ExitPlanMode hook that saves it (cmd/logos/activity.go's recordActivity)
...
func runPlans(args []string) error {
	...
	if project == "" {
		return fmt.Errorf("usage: logos plans <project>")
	}
