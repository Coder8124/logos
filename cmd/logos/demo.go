...
// `logos demo` — the ninety seconds that show what this is for.
//
...
func runDemo(args []string) error {
	...
	fmt.Println("    logos bootstrap --dry-run")
	fmt.Println()
	return nil
}
...
func demoRemember(db *sql.DB, embed *provider.Provider, model, text string) (string, error) {
	...
	if r.Created() {
		return fmt.Sprintf("✓ Logos · stored in logos — memory #%d", r.ID), nil
	}
...
func openEventsAt(vault string) (*index.Index, error) {
	if err := vaultpkg.MkdirPrivate(filepath.Join(vault, ".logos")); err != nil {
		return nil, err
	}
	old := os.Getenv("LOGOS_VAULT")
	os.Setenv("LOGOS_VAULT", vault)
	defer os.Setenv("LOGOS_VAULT", old)
	return openEvents()
}
