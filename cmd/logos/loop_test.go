...
// `logos loop` with no arguments is the list, and nothing says so. Every other
// listing verb in this CLI takes `list`, the help line reads "logos loop
// [add|done|drop]" — which names three verbs, none of them the one that shows
// you your commitments — and `logos loop list` answered with a usage error.
...
func TestLoopListIsTheListRatherThanAUsageError(t *testing.T) {
	...
	if !strings.Contains(out, text) {
		t.Errorf("`logos loop list` did not list the loop:\n%s", out)
	}
	...
	if bare != out {
		t.Errorf("`logos loop list` and `logos loop` disagree:\n%q\nvs\n%q", out, bare)
	}
}
...
func TestClosingALoopThatDoesNotExistSaysSo(t *testing.T) {
	...
	for _, verb := range []string{"done", "drop"} {
		if err := commitmentCmd([]string{verb, "99"}); err == nil {
			t.Errorf("`logos loop %s 99` reported success for a loop that does not exist", verb)
		}
		if err := commitmentCmd([]string{verb}); err == nil {
			t.Errorf("`logos loop %s` with no id reported success", verb)
		}
	}
}
