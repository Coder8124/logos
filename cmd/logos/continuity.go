...
// runContinuity answers the question `logos doctor` cannot: not "is logos
...
func runContinuity(args []string) error {
	...
	if len(report) == 0 {
		...
		fmt.Println("run `logos note <project> ...` or `logos checkpoint <project> ...` to start one.")
		return nil
	}
	...
	for _, pc := range report {
		...
		if pc.Abandoned > 0 {
			fmt.Printf("  %-24s   %d abandoned session(s) — see `logos sessions %s`\n", "", pc.Abandoned, pc.Project)
		}
	}
	fmt.Printf("\n%d project%s · %d quiet or never checkpointed\n",
		len(report), pluralS(len(report)), quiet)
	return nil
}
