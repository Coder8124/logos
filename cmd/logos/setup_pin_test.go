...
// npxSetup runs setup as a logos executing from npm's npx cache, after
...
func npxSetup(t *testing.T, prepare func(home string), args ...string) (home, out string, got setup.Server) {
	...
	fakeProgram(t, cached, "logos", `echo "logos 0.4.3 darwin/arm64 go1.26"`)
	prepare(home)

	oldExe, oldHosts, oldCheck := executable, detectHosts, integrationChecks
	executable = func() (string, error) { return filepath.Join(cached, "logos"), nil }
...
func TestAnNpxSetupRegistersACopyOfLogosInsteadOfNpx(t *testing.T) {
...
// ~/.local/bin/logos may be another program by that name. Overwriting it to
// save a network round trip would break something the user installed.
func TestAnNpxSetupDoesNotOverwriteAnotherProgramCalledLogos(t *testing.T) {
	home := t.TempDir()
	pinDir := filepath.Join(home, ".local", "bin")
	fakeProgram(t, pinDir, "logos", `echo "logos-games 2.1"`)
	before, _ := os.ReadFile(filepath.Join(pinDir, "logos"))

	err := pinBinary("/nonexistent/logos", filepath.Join(pinDir, "logos"))
	if err == nil || !strings.Contains(err.Error(), "not logos") {
		t.Errorf("pinBinary over someone else's logos returned %v", err)
	}
	if after, _ := os.ReadFile(filepath.Join(pinDir, "logos")); string(after) != string(before) {
		t.Errorf("another program's logos was overwritten")
	}
}
...
// newerCopy puts a logos that reports 0.4.9 where setup pins its copy.
...
func pinnedReports(t *testing.T, home string) string {
	t.Helper()
	raw, err := os.ReadFile(filepath.Join(home, ".local", "bin", "logos"))
	if err != nil {
		t.Fatal(err)
	}
	return string(raw)
...
func TestAnOlderNpxSetupKeepsANewerPinnedCopyUnderYes(t *testing.T) {
	...
	if got.Bin != filepath.Join(home, ".local", "bin", "logos") {
...
func TestAChosenDowngradeReplacesThePinnedCopy(t *testing.T) {
	...
	for name, run := range map[string]func() (string, string){
		...
		home, out := run()
		if !strings.Contains(pinnedReports(t, home), "logos 0.4.3") {
